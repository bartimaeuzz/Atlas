...
async function seed() {
  ...
  const tableNames = [
    ...
    "employee_positions", "employees", "online_platforms", "positions", "sections",
    "restaurant_settings",
  ];
  ...
  const [bartender] = await db.insert(positions).values({ name: "Bartender", category: "FOH", tipPoolGroup: "POOL_1_DINE_IN", defaultTipPointValue: 1.0 }).returning();
  const [host] = await db.insert(positions).values({ name: "Host", category: "FOH", tipPoolGroup: "POOL_1_DINE_IN", defaultTipPointValue: 1.0 }).returning();
  // NOTE: Host is ALSO a Pool 2 participant. Since tipPoolGroup is one value
  // per Position, and Host spans both pools, we model this with a second
  // Position row "Host (Takeout/Online)" sharing the same employees via
  // EmployeePosition, rather than allowing one Position to belong to two
  // pools at once. Keeps the pool-split calc simple (one pool per position).
  const [hostPool2] = await db.insert(positions).values({ name: "Host (Takeout/Online)", category: "FOH", tipPoolGroup: "POOL_2_TAKEOUT_ONLINE", defaultTipPointValue: 1.0 }).returning();
  const [runner] = await db.insert(positions).values({ name: "Runner", category: "FOH", tipPoolGroup: "POOL_1_DINE_IN", defaultTipPointValue: 1.0 }).returning();
  const [busser] = await db.insert(positions).values({ name: "Busser", category: "FOH", tipPoolGroup: "POOL_1_DINE_IN", defaultTipPointValue: 1.0 }).returning();
  const [operator] = await db.insert(positions).values({ name: "Operator", category: "FOH", tipPoolGroup: "POOL_2_TAKEOUT_ONLINE", defaultTipPointValue: 1.0 }).returning();
  const [packer] = await db.insert(positions).values({ name: "Packer", category: "FOH", tipPoolGroup: "POOL_2_TAKEOUT_ONLINE", defaultTipPointValue: 1.0 }).returning();
  const [bagHandler] = await db.insert(positions).values({ name: "Bag Handler", category: "FOH", tipPoolGroup: "POOL_2_TAKEOUT_ONLINE", defaultTipPointValue: 1.0 }).returning();
  const [manager] = await db.insert(positions).values({ name: "Manager", category: "FOH", tipPoolGroup: "NONE", alwaysVisibleInRoster: true, earningsHiddenFromStaff: true, defaultTipPointValue: null }).returning();
  const [floorManager] = await db.insert(positions).values({ name: "Floor Manager", category: "FOH", tipPoolGroup: "NONE", alwaysVisibleInRoster: true, earningsHiddenFromStaff: true, defaultTipPointValue: null }).returning();
  ...
  const [deliveryGuy] = await db.insert(positions).values({ name: "Delivery Guy", category: "FOH", tipPoolGroup: "POOL_3_DELIVERY", defaultTipPointValue: null }).returning();
  const [chef] = await db.insert(positions).values({ name: "Chef", category: "BOH", tipPoolGroup: "NONE", defaultTipPointValue: null }).returning();
  const [lineCook] = await db.insert(positions).values({ name: "Line Cook", category: "BOH", tipPoolGroup: "NONE", defaultTipPointValue: null }).returning();
  ...
  await db.insert(employeePositions).values([
    ...
    { employeeId: erika.id, positionId: hostPool2.id, tipPointValue: 1.0 }, // Erika also shares Pool 2 as host
  ...
  await db.insert(shiftRosterEntries).values([
    ...
    { shiftId: dinnerShift.id, employeeId: erika.id, positionId: host.id },
    { shiftId: dinnerShift.id, employeeId: erika.id, positionId: hostPool2.id },
