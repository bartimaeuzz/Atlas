...
### Standing checks for design/plan docs (added 2026-08-18, both from a 2nd-occurrence recurrence — see `feedback_atlas_verify_against_live_code.md` and `feedback_atlas_granularity_scope_creep.md` in project memory)

Run both of these explicitly whenever the artifact under review is a plan or design doc rather than a code diff — they've each caused a real fix twice already, which is what promoted them from one-off findings to standing checks:

- **Live-code verification.** A design/foundation addition reviewed from memory or a plan document alone is not verified — only checked for internal consistency. Before it can be marked CONFIRMED, it must be checked against the actual current repo code (clone `main`, grep for existing implementations of what's being proposed). This class of gap has already cost one full corrective pass (2026-08-16, the `design-system-v2` foundation check found 5 real gaps invisible from memory alone: a raw `window.confirm()`, a duplicated ad hoc modal, inconsistent `<select>` styling, hardcoded colors outside the token system) and recurred once since (2026-08-18, UI Design's tier-2 additions). If the reviewing session genuinely has no repo access, say so explicitly and mark the artifact DESIGNED-NOT-CONFIRMED rather than shipping it as verified.
