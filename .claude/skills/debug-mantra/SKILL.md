...
Oliver reports bugs by clicking through the live app himself — his reports are ground truth (introduction.md §Your role) but usually arrive as a symptom description, not a repro script. Step 1 here means: before proposing any fix, either reproduce the bug directly against the seed/test data in the dev DB, or get specific enough steps from Oliver to do so — don't patch based on a guessed cause. This matters most for anything touching money math (tip pools, wage calc) or roster-visibility logic, where Atlas's mistake log (HANDOFF.md §7) shows most past errors came from skipping straight to a plausible-looking fix. Never reproduce or instrument against the live Turso `atlas-prod` database — use local/dev data only, per introduction.md's credential and data-safety rules.
