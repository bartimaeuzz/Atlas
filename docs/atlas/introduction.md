...
Read this file first, every session, before touching anything else. It's the operating charter for this project — the rules that don't change session to session. For what's currently true (latest shipped feature, deployment state, open questions), the source of truth is now the **Current State block at the top of `MEMORY.md`'s index** (auto-loaded every session, no extra read needed) plus a live git check — see "How to start every session" below. `HANDOFF.md` in this folder is deep-history reference only (rules, sandbox gotchas, the full mistake log) — genuinely useful for "why was this decided," but its own state section is hand-updated and goes stale between sessions, so don't treat it as current. (Oliver's work style, 2026-08-17/18: he closes a session and starts a fresh one after each feature to save tokens — this file's job is to make a brand-new session land on accurate ground truth fast, without re-reading a large stale doc.)
...
5. Only pull `HANDOFF.md` or older memory topic files on demand, when the task actually needs deep history/reasoning — not as a default full read every session.
...
- **Project memory:** `MEMORY.md`'s index (including its Current State block) auto-loads every session — this is the state source of truth. Individual topic files are deep archive, pulled on demand via `project_memory_read` when you need exact history/reasoning.
- **This folder:** working files Oliver has shared directly — DNA/reference spreadsheets, screenshots, this document, `HANDOFF.md`.
